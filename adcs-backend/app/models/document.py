...
class Document(Base):
    ...
    so_ky_hieu = Column(String(255), nullable=True) # Map từ so_hieu_van_ban
    trich_yeu = Column(Text, nullable=True)
    hinh_thuc = Column(String(100), nullable=True)
    loai_van_ban_text = Column(String(100), nullable=True) # Map từ loai_van_ban.nhan
    don_vi_ban_hanh = Column(Text, nullable=True) # Map từ co_quan_ban_hanh
    nguoi_ky = Column(String(255), nullable=True)
    chuc_vu_nguoi_ky = Column(String(255), nullable=True)
    ngay_van_ban = Column(Date, nullable=True) # Map từ ngay_ban_hanh
    ...
    noi_nhan = Column(JSONB, nullable=True) # Mảng string
    can_cu_phap_ly = Column(JSONB, nullable=True) # Mảng string
    yeu_cau_hanh_dong = Column(Text, nullable=True)

    # --- Kết quả phân tích AI & Trạng thái ---
    status = Column(String, nullable=True)
    summary = Column(Text, nullable=True) # Map từ tom_tat
    key_points = Column(JSONB, nullable=True) # Mảng string
    confidence = Column(Float, nullable=True) # Map từ diem_tin_cay tổng
    ...
    de_xuat_xu_ly = Column(JSONB, nullable=True) # Map từ de_xuat_thoi_han_xu_ly object
    goi_y_phong_ban = Column(JSONB, nullable=True) # Map từ goi_y_phong_ban object
